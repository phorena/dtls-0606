...
impl Hub2 {
    ...
    pub async fn register(&self, conn: Arc<dyn Conn + Send + Sync>) {
        ...
        tokio::spawn(async move {
            let _ = Hub2::read_loop(conn.remote_addr().await.unwrap(), channel_tx, conns, conn).await;
        });
    }

    /// register adds a new conn to the Hub
    pub async fn get_conn(&self, socket_addr: SocketAddr) -> Option<Arc<dyn Conn + Send + Sync>> {
